...
const AVAILABLE_COUNTRIES = ALL_COUNTRY_CODES
  .map((code) => ({ name: regionNames.of(code) || code, code, flag: codeToFlagEmoji(code) }))
...
const profileSlice = createSlice({
    ...
    reducers: {
        ...
        setSearchTerm: (state, action) => {
            state.nationalities.searchTerm = action.payload;
        },
        selectNationality: (state, action) => {
            state.nationalities.selectedNationality = action.payload;
        },
        ...
        editNationality: (state) => {
            ...
            state.nationalities.searchTerm = '';
            state.nationalities.selectedNationality = null;
        },
        ...
        addOtherAddress: (state) => {
            const newId = Math.max(...state.residencies.otherAddresses.map(addr => addr.id), 0) + 1;
            state.residencies.otherAddresses.push({ id: newId, value: '' });
        },
        updateOtherAddress: (state, action) => {
            const { id, value } = action.payload;
            const address = state.residencies.otherAddresses.find(addr => addr.id === id);
            if (address) {
                address.value = value;
            }
        },
        removeOtherAddress: (state, action) => {
            state.residencies.otherAddresses = state.residencies.otherAddresses.filter(
        ...
        setCurrentFile: (state, action) => {
            state.uploadedFiles.currentFile = { ...state.uploadedFiles.currentFile, ...action.payload };
        },
